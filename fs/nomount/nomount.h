...
static void **nm_art_find_child(struct art_node *n, u8 c)
{
    ...
    switch (n->type) {
        ...
        case ART_NODE48: {
            ...
            u8 index = n48->child_index[c];
            if (index) return &n48->children[index - 1];
            break;
        }
        case ART_NODE256: {
...
static void nm_art_add_child(void **ref, u8 c, void *child)
{
    ...
    if (n->type == ART_NODE4) {
        ...
        n16 = kzalloc(sizeof(*n16), GFP_KERNEL);
        n16->n = n4->n;
        ...
        *ref = n16;
        ...
        n48 = kzalloc(sizeof(*n48), GFP_KERNEL);
        n48->n = n16->n;
        ...
        *ref = n48;
        ...
        n256 = kzalloc(sizeof(*n256), GFP_KERNEL);
        n256->n = n48->n;
...
static void *nm_art_first_child(struct art_node *n)
{
    ...
    children = n->type == ART_NODE48 ? ((struct art_node48 *)n)->children : ((struct art_node256 *)n)->children;
    capacity = n->type == ART_NODE48 ? 48 : 256;
    for (i = 0; i < capacity; i++) if (children[i]) return children[i];
    return NULL;
}
...
static void nm_tree_insert(struct nomount_rule *new_rule)
{
    ...
    while ((node = *node_ref)) {
        if (ART_IS_LEAF(node)) {
            ...
            n4 = kzalloc(sizeof(*n4), GFP_KERNEL);
            n4->n.type = ART_NODE4;
            n4->n.num_children = 2;

            existing_key = nm_get_vpath(existing);
            max_cmp = min_t(int, existing->v_len, new_rule->v_len) - depth;
...
static void nm_art_free_tree(void *node)
{
    ...
    if (n->type == ART_NODE4) {
        struct art_node4 *n4 = (struct art_node4 *)n;
        for (i = 0; i < n->num_children; i++) nm_art_free_tree(n4->children[i]);
    } else if (n->type == ART_NODE16) {
        struct art_node16 *n16 = (struct art_node16 *)n;
        for (i = 0; i < n->num_children; i++) nm_art_free_tree(n16->children[i]);
    } else if (n->type == ART_NODE48) {
        struct art_node48 *n48 = (struct art_node48 *)n;
        for (i = 0; i < 256; i++) {
...
static inline int nm_uid_add(uid_t target)
{
    ...
    if ((old = rcu_dereference_protected(nomount_uids, lockdep_is_held(&nomount_rwsem)))) {
        for (i = 0; i < (count = old->count); i++) if (old->uids[i] == target) return -EEXIST;
    }
...
static inline int nm_uid_del(uid_t target)
{
    ...
    int count, target_idx = -1, i;

    if (!(old = rcu_dereference_protected(nomount_uids, lockdep_is_held(&nomount_rwsem)))) return -ENOENT;
    for (i = 0; i < (count = old->count); i++) if (old->uids[i] == target) { target_idx = i; break; }
